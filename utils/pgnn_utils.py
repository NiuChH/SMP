# reference: https://github.com/JiaxuanYou/P-GNN
...
def get_random_edge_mask_link_as_neg(mask_link_positive, num_nodes, num_negative_edges):
    return torch.randint(0, num_nodes, (2, num_negative_edges), dtype=torch.long).cpu()


# exact version, slower
def get_edge_mask_link_negative(mask_link_positive, num_nodes, num_negative_edges, approximate=False):
    if approximate:  # faster
        return get_edge_mask_link_negative_approximate(mask_link_positive, num_nodes, num_negative_edges)
...
def resample_edge_mask_link_negative(data):
    data.mask_link_negative_train = get_edge_mask_link_negative(data.mask_link_positive_train, num_nodes=data.num_nodes,
                                                                num_negative_edges=data.mask_link_positive_train.shape[
                                                                    1])
...
def get_link_mask(data, remove_ratio=0.2, resplit=True, infer_link_positive=True, infer_link_negative=True):
    if resplit:
...
def pre_compute_dist_data(edge_index, num_nodes, approximate=0):
    """
